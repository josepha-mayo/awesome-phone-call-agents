...
class Client(Protocol):
    """The surface Afterword needs. Implemented by the real and dry-run clients."""
...
class HttpClient:
    ...
    def create_call(self, payload: dict[str, Any], idempotency_key: str) -> dict[str, Any]:
        ...
        # attempt, so a retried run can never ring a bereavement team twice.
...
class DryRunClient:
    ...
    This is the default so that running Afterword by accident cannot ring
    anybody.
    """
...
def wait_for_result(
    ...
    A timeout is not a refusal. It leaves the outcome unresolved, which the
    caller must treat as UNREACHED rather than as an answer.
    """
